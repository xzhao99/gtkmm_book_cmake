...
const std::array<TransitionTypeStruct, 3> transitionTypes = {
    TransitionTypeStruct{"none", "None"}, TransitionTypeStruct{"crossfade", "Fade"},
...
ExampleAppPrefs::ExampleAppPrefs(BaseObjectType* cobject,
    ...
    m_settings->bind<Glib::ustring, unsigned int>(
        "transition", m_transition->property_selected(), Gio::Settings::BindFlags::DEFAULT,
    ...
    m_settings->signal_changed("transition")
        .connect(sigc::mem_fun(*this, &ExampleAppPrefs::on_transition_setting_changed));
...
ExampleAppPrefs::map_from_ustring_to_int(const Glib::ustring& transition) {
