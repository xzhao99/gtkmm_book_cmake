...
void ExampleAppWindow::on_search_text_changed() {
    ...
    if (!view) {
        std::cout << "ExampleAppWindow::on_search_text_changed(): No visible text view."
                  << std::endl;
        return;
    }
    ...
    if (buffer->begin().forward_search(text, Gtk::TextSearchFlags::CASE_INSENSITIVE, match_start,
                                       match_end)) {
...
void ExampleAppWindow::on_reveal_child_changed() { update_words(); }
...
void ExampleAppWindow::update_words() {
    ...
    for (const auto& word : words) {
        ...
        row->signal_clicked().connect(
            sigc::bind(sigc::mem_fun(*this, &ExampleAppWindow::on_find_word), row));
        m_words->append(*row);
    }
}
